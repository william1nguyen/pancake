...
const command = new SlashCommandBuilder()
  ...
  .addBooleanOption((option) =>
    option.setName("webhook").setDescription("Is use webhook ?"),
  );
...
const execute = async (interaction: ChatInputCommandInteraction) => {
  ...
  const isUseWebhook =
    optional(interaction.options.getString("webhook")) || false;
  const webhook = isUseWebhook ? user.webhook : undefined;

  if (isUseWebhook && !webhook) {
    const botResponse = createResponse(BotResponse.WebhookMissed);
    interaction.reply(botResponse);
    return;
  }
